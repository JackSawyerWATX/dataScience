...
- `correlation_vs_causality.py`: demonstrates that correlation does not imply causation with a scatter plot; prints the correlation matrix and prompts to save an optional PNG.
- `statistics_correlation_matrix.py`: computes a cleaned correlation matrix and displays a heatmap (coerces numeric columns and handles space thousands separators).
...
- `simple_arithmetic.py`: tiny script demonstrating basic arithmetic operations and simple function examples for quick testing.
- `plot.png`: example output image generated by the plotting examples.
- `rounding_decimals.py`: examples demonstrating rounding numbers and controlling decimal places using NumPy and Python's built-in `round()`.
